...
//     try {
//       const { data } = await axios.get("/api/school-programs");
//       setPrograms(data);
//     } catch (error) {
...
//     const { title, description, price, image } = newProgram;
...
//       );
//       fetchPrograms();
//       setNewProgram({ title: "", description: "", price: "", image: "" });
//     } catch (error) {
...
//       const config = { headers: { Authorization: `Bearer ${token}` } };
//       await axios.put(
//         `/api/school-programs/${editProgram._id}`,
//         {
//           title: editProgram.title,
//           description: editProgram.description,
//           price: Number(editProgram.price),
...
//       }
//       const config = { headers: { Authorization: `Bearer ${token}` } };
//       await axios.delete(`/api/school-programs/${id}`, config);
...
//                         </div>
//                       </td>
//                       <td className="px-6 py-4">${program.price}</td>
//                       <td className="px-6 py-4">
...
const SchoolProgramsAdmin = () => {
  ...
  const addProgram = async () => {
    ...
    if (!title || !description || !image || !youtubeLink) {
      console.error("All fields (title, description, image, YouTube link) are required!");
      return;
    }
    try {
      const token = localStorage.getItem("token");
